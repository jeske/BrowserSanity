...
This is the very first prompt I used to kick off this project. I am preserving it here, both for posterity, and for the purpose of helping others understand a bit how I construct prompts for the AI. There was lots more than this that went into getting the app done but the core functionality (not counting the UI!) took under 2 hours to get working with the AI. Which is pretty astounding, especially considering it IS the installer/uninstaller/daemon/UI/and tool.

I did get completely rat-hole-ed on the UI. I wanted a simple no-dependency project, so I started with raw win32 GUI, but the message pump and state management was confusing the AI. So did some research and picked Nuklear as a very lightweight header-only GUI kit, that is not only easy on the eyes but also fairly configurable. I think it's a great tool, but the C-style-void-everything and the lack of multiple window management (which we absolutely have to have) made it kind of a headache for the AI and I to wrap our heads around. This is a key lesson in AI coding, which is that well-typed libraries are much easier for the AI to work with than things with void* and/or abstractions with alot of state. I think I would have been done 6-8 hours earlier if I had used FLTK or nana, but I wasn't super thrilled about how they looked, and I was already knee deep in Nuklear. So we ended up implementing a C++ Windows-Manager around Nuklear, which took a while to get right. At some point, I may make a full C++ typed wrapper for Nuklear, as it would be much easier for the AI to code with this than the current interface.
